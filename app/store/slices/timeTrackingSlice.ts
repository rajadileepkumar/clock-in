...
export type TimeSession = {
  ...
  status: "ACTIVE" | "COMPLETED" | "PAUSED" | "PENDING" | "REJECTED" | "APPROVED";
  task?: string;
  notes?: string;
  location?: {
    lat: number;
...
const timeTrackingSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchUserSessions.fulfilled, (state, action) => {
      ...
      state.activeSession = action.payload.find((s: TimeSession) => s.status === "ACTIVE") || null;
      state.isClockedIn = !!state.activeSession;
      state.loading = false;
      
      // Build records from sessions for Header component
      state.records = action.payload.reduce((acc: TimeRecord[], session: TimeSession) => {
        // Add clock-in record
        acc.push({
